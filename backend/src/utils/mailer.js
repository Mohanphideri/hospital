...
function prescriptionEmailHtml({ name, appointmentCode, billNumber, hasBillAttachment }) {
  const attachmentsText = hasBillAttachment
    ? "we've attached a copy of your prescription and your bill (both PDF) to this email"
    : "we've attached a copy of your prescription (PDF) to this email";
  return `
  <div style="background:#F4F5F7;padding:32px 16px;font-family:Arial,Helvetica,sans-serif;">
    <table role="presentation" width="100%" style="max-width:480px;margin:0 auto;background:#FFFFFF;border-radius:16px;overflow:hidden;border:1px solid #E5E7EB;">
      <tr>
      ...
      <tr>
        <td style="padding:32px;">
          <h1 style="font-size:18px;color:#0F1F3D;margin:0 0 12px;">${hasBillAttachment ? "Your bill and prescription are attached" : "Your prescription is attached"}</h1>
          <p style="font-size:14px;color:#475569;line-height:1.6;margin:0 0 20px;">
            ${name ? `Hi ${name}, your` : 'Your'} bill for appointment <strong>${appointmentCode || '-'}</strong> has been generated, and ${attachmentsText}.
          </p>
          ...
          <div style="background:#FCE7EB;border-radius:12px;padding:18px 20px;margin-bottom:8px;">
            <p style="font-size:13px;color:#0F1F3D;font-weight:700;margin:0 0 6px;">🔒 ${hasBillAttachment ? 'These PDFs are' : 'This PDF is'} password-protected</p>
            <p style="font-size:13px;color:#475569;line-height:1.6;margin:0;">
              To open ${hasBillAttachment ? 'them' : 'it'}, enter the <strong>last 4 digits of the mobile number registered with your HeartStone account</strong> as the password.
            </p>
          </div>
          <p style="font-size:12px;color:#94A3B8;line-height:1.6;margin:16px 0 0;">
            If this wasn't you, or your mobile number has changed, please contact reception before opening the attachment${hasBillAttachment ? 's' : ''}.
          </p>
        </td>
      </tr>
      <tr>
...
export const sendPrescriptionPdfEmail = async (
  toEmail,
  { patientName, appointmentCode, billNumber, pdfBytes, fileName, billPdfBytes, billFileName }
) => {
  if (!toEmail) return { skipped: true, reason: 'no-email-on-file' };
  if (!isEmailConfigured()) {
    console.log(`[EMAIL SKIPPED - not configured] Would send prescription${billPdfBytes ? ' + bill' : ''} PDF to ${toEmail} for bill ${billNumber}`);
    return { skipped: true, reason: 'not-configured' };
  }
  ...
  if (billPdfBytes) {
    attachments.push({
      name: billFileName || 'bill.pdf',
      base64: Buffer.from(billPdfBytes).toString('base64'),
    });
  }
  ...
  try {
    await sendEmail({
      ...
      html: prescriptionEmailHtml({ name: patientName, appointmentCode, billNumber, hasBillAttachment: Boolean(billPdfBytes) }),
      attachments,
    });
    return { skipped: false, sent: true };
  } catch (err) {
