...
export const applyForLeave = async (req, res) => {
  try {
    ...
    // A staff member whose most recent request for these dates was rejected
    // can only reapply starting the next day - mirrors the front-end notice.
    ...
    // Prevent overlapping leave requests for the same staff member (pending or approved).
...
// Admin: every leave request that has already been decided (approved or
// rejected), across all staff - powers the "Leave history" tab so admin can
// see past decisions without them cluttering the pending queue.
...
// Admin: appointments that would be affected if this (doctor's) leave is approved -
// booked/completed-pending appointments for that staff member falling inside the
// leave date range. Only meaningful for doctors, but harmless to compute for anyone.
