...
// --- Atomic bed claim/release helpers ---------------------------------
// Every bed-status change below used to be: read the ward, check
// bed.status in JS, then ward.save(). Two concurrent admissions to the
// same bed within the same moment could both pass the "is it vacant?"
// check before either write landed - a classic read-then-write race. These
// helpers use a single findOneAndUpdate whose *query* also requires the
// bed to still be in the expected state, so the check-and-set happens
// atomically at the database level: only one concurrent caller can ever
// win a claim on a given bed.
...
async function releaseBed(wardId, bedId) {
  // Deliberately does not require the bed to currently be 'occupied' -
  // discharge/transfer-away should always succeed in freeing the bed even
  // if its status was already inconsistent for some other reason; the
  // important atomicity guarantee is on the *claiming* side above.
  return Ward.findOneAndUpdate(
...
// Admin: edit a ward's own details (name, type, floor, department).
...
// Admin: edit a bed's own details - number, daily charge, or ICU care level.
// (Occupancy/maintenance status is changed via updateBedStatus below.)
...
// Admin: remove a bed from a ward. Only allowed while it's vacant, so an
// admitted patient's bed can never disappear out from under them.
export const deleteBed = async (req, res) => {
  try {
    ...
    // Atomic: $pull only removes the bed if it's still not occupied at
    // write time - same race class as above (someone could admit into it
...
// Admin: remove an entire ward. Only allowed while it has no beds left, so
// there's no risk of orphaning an admitted patient's bed reference.
...
export const updateBedStatus = async (req, res) => {
  try {
    ...
    // Atomic condition-checked write: only succeeds if the bed is still not
    // occupied at the moment of the update, closing the same race class as
    // admission (someone could admit into this bed between our read above
    // and the write, if we'd used ward.save() here instead).
...
export const admitPatient = async (req, res) => {
  try {
    ...
    // Existence check first (for a clean 404) - the actual vacancy decision
    // happens atomically in claimBed() below, not from this read.
    ...
    if (!claimedWard) {
      // Bed existed a moment ago but is no longer vacant - either someone
      // else just admitted into it, or it was put into maintenance.
    ...
    try {
      ...
      // Compensate: we successfully claimed the bed but failed to record the
      // admission (validation error, DB hiccup, etc.) - release it back to
      // vacant rather than leaving a bed permanently stuck "occupied" with
      // no admission record pointing at it.
...
export const transferBed = async (req, res) => {
  try {
    ...
    // Claim the destination FIRST, atomically. If this fails (someone else
    // just took that bed), we bail out having touched nothing - the
    // patient's current bed is untouched and nothing needs to be undone.
    ...
    // Now free the old bed. This is a same-patient release (nobody else can
    // be racing for it since it's still tied to this admission) so no
    // vacancy condition is needed here - just clear it.
    try {
      await releaseBed(admission.wardId, admission.bedId);
    } catch (err) {
      // Extremely unlikely (a DB error, not a race), but if it happens,
      // undo the destination claim rather than leaving two beds marked
      // occupied for one patient.
...
// Also returns a billing suggestion (receptionist/admin generate the actual IPD
...
// Receptionist / admin: generate the IPD stay bill once discharged.
export const createIpdBill = async (req, res) => {
  try {
    ...
    const bill = await Bill.create({
      ...
      medicinesTotal,
      consultationFee: fee,
      // IPD stay bills aren't appointment-based, so the flat appointment fee
      // (schema default is ₹500, meant for OPD billing) doesn't apply here.
      appointmentFee: 0,
      totalAmount,
