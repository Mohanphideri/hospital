...
import { ArrowLeft, CalendarCheck, Smartphone, Lock, CheckCircle2, Building2, Clock, Phone, RotateCw, User, Mail, Video } from "lucide-react";
import Spinner from "../../../components/ui/Spinner";
import HeartMark from "../../../components/ui/HeartMark";
import Modal from "../../../components/ui/Modal.jsx";
import { useAuth } from "../../../contexts/AuthContext.jsx";
import { initSocket } from "../../../utils/socket.js";
import { authService, patientService, departmentService, scheduleService, appointmentService, captchaService } from "../../../services/index.js";
...
import CaptchaField from "../../../components/forms/CaptchaField.jsx";
...
export default function BookAppointment() {
  ...
  const [date, setDate] = useState(() => getClinicTodayString());
  const [slots, setSlots] = useState([]);
  const [slotsLoading, setSlotsLoading] = useState(false);
  const [confirmed, setConfirmed] = useState(null);
  // "in-person" (default) or "online" - whether this is a video consultation.
  ...
  const runBooking = async (time) => {
    setError("");
    try {
      const payload = { departmentId, consultationType };
      if (time) {
  ...
  useEffect(() => {
    if (consultationType === "online" && isGeneralDepartment) {
      setDepartmentId("");
      setSlots([]);
    }
  }, [consultationType, isGeneralDepartment]);
    ...
    <div className="min-h-screen bg-[radial-gradient(circle_at_top_left,_rgba(200,16,46,0.10),_transparent_20%),_radial-gradient(circle_at_bottom_right,_rgba(15,31,61,0.06),_transparent_24%)] flex flex-col">
      ...
      <main className="flex-1 flex items-center justify-center px-4 py-10">
        <div className="w-full max-w-lg rounded-[1.75rem] bg-white border border-mist shadow-[0_40px_120px_-40px_rgba(15,31,61,0.25)] p-8">
          ...
          {step === "book" && !typeChosen && (
            <div className="space-y-5">
              <p className="text-sm text-slate-soft">How would you like to consult?</p>
              ...
              <button
                ...
                className="w-full rounded-full bg-crimson px-6 py-3 text-sm font-semibold text-white hover:bg-crimson-dark transition-colors"
              >
                Continue
              </button>
            </div>
          ...
          {step === "book" && typeChosen && (
            <div className="space-y-5">
              <button
                ...
                className="inline-flex items-center gap-2 rounded-full bg-mist/70 px-3.5 py-1.5 text-xs font-semibold text-ink hover:bg-mist transition-colors"
              >
              ...
              <label className="block space-y-2">
                ...
                <div className="relative">
                  ...
                  >
                    ...
                    {departments
                      // General has no doctor/time assigned up front (the front
                      // desk does that later), so there's nothing to video-call
                      // into - it can never be an online consultation. Rather
                      // than silently downgrading the person's chosen type to
                      // in-person once they pick it, just don't offer it while
                      // they're on the online-consultation path.
                      .filter((d) => !d.isGeneral || consultationType !== "online")
                      .map((d) => (
                ...
                {consultationType === "online" && (
                  <span className="block text-xs text-slate-soft">General isn't available for online consultations — it needs a specific department so a doctor and time can be assigned upfront.</span>
                )}
              </label>
          ...
          {step === "done" && (
            <div className="flex flex-col items-center gap-4 py-4 text-center">
              ...
              <p className="text-sm text-slate-600 leading-relaxed">
                ...
                {consultationType === "online" && confirmed?.doctorId && (
                  <> Join from the "Online consultations" tab in your account when it's time.</>
                )}
              </p>
              ...
              <div className="flex flex-col sm:flex-row gap-3 mt-2 w-full">
                ...
                <button
                  onClick={() => navigate(consultationType === "online" && confirmed?.doctorId ? "/patient/consultations" : "/patient")}
                  className="flex-1 rounded-full bg-navy px-5 py-2.5 text-sm font-semibold text-white hover:bg-navy-light transition-colors"
                >
                  {consultationType === "online" && confirmed?.doctorId ? "Go to online consultation" : "View my account"}
                </button>
              </div>
            </div>
          )}
        ...
        {pendingConfirm && (
          <div className="space-y-5">
            <div className="rounded-xl bg-mist/70 p-4 text-sm text-ink space-y-1.5">
              ...
              <div><span className="text-slate-soft">Visit type:</span> <span className="font-semibold">{consultationType === "online" ? "Online consultation (video)" : "In-person visit"}</span></div>
              {pendingConfirm.time ? (
                <>
