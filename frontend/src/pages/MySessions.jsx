...
// policy (multiple device sessions allowed, each independently revocable).
// Reachable at /account/sessions from any authenticated portal.
export default function MySessions() {
  ...
  // Revoking a session always hits the server first. If the session being
  // revoked is THIS device, the server-side refresh token is now dead the
  // moment that call succeeds - so we immediately clear local auth state and
  // leave for the landing page too, rather than leaving the person sitting
  // on a page that will just 401 on its next request.
  const handleRevoke = async (session) => {
    ...
    try {
      ...
      // api.js interceptor already surfaced an error toast
    } finally {
      setBusyId(null);
    }
  };

  // "Sign out of all devices" - every other session first, then this one,
  // then leave for the landing page immediately since this browser's
  // session is now dead too.
  ...
  // "Sign out of all other devices" - keeps this session alive, so there's
  // nothing to navigate away from; just refresh the list.
