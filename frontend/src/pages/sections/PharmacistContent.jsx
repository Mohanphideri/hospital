// Split out of ../Section.jsx (see that file's renderPharmacist usage) to keep Section.jsx a manageable size. All state lives in Section.jsx; this file receives everything it
  ...
  if (section === "inventory") {
    ...
    return <div className="space-y-4">
        ...
        return <DataCard key={med._id} title={med.name} subtitle={`${med.totalQuantity} ${med.unit} in stock across ${med.batches?.length || 0} batch${med.batches?.length !== 1 ? "es" : ""}`} badge={<StatusBadge status={med.isAvailable ? "Available" : "Unavailable"} tone={med.isAvailable ? "success" : "danger"} />}>
              ...
              return <div key={b._id} className="grid grid-cols-4 gap-3 rounded-xl bg-mist px-4 py-2.5 text-sm">
