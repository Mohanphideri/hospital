// Split out of ../Section.jsx (see that file's renderIpd usage) to keep Section.jsx a manageable size. All state lives in Section.jsx; this file receives everything it
...
export function renderIpdWorkspaceImpl(role, { admissionStatusFilter, admissions, admitForm, admitStatus, departments, dischargeDrafts, doctors, ipdActionStatus, ipdBillDrafts, loading, newBedForm, newBedStatus, newWardForm, newWardStatus, setAdmissionStatusFilter, setAdmitForm, setDischargeDrafts, setIpdBillDrafts, setNewBedForm, setNewWardForm, setTransferChoice, submitAddBed, submitAdmitPatient, submitCreateWard, submitDeleteBed, submitDeleteWard, submitDischarge, submitIpdBill, submitSetBedStatus, submitTransferBed, transferChoice, wards }) {
  ...
  return <div className="space-y-8">
        ...
        {/* Add a bed to a ward (admin only) */}
        {role === "admin" && wards.length > 0 && <div>
        ...
        <div>
          ...
          {loading ? <SkeletonList count={3} /> : wards.length === 0 ? <EmptyRow icon={BedDouble} title="No wards yet">No wards have been set up yet.</EmptyRow> : <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
              {wards.map(w => <DataCard key={w._id} title={w.name} subtitle={`${w.type} · Floor ${w.floor || "—"}`} actions={role === "admin" && w.beds.length === 0 ? <button onClick={() => submitDeleteWard(w._id)} className="rounded-full border border-red-200 bg-red-50 px-3 py-1 text-[11px] font-semibold text-red-700 hover:bg-red-100 transition-colors">
                        Remove ward
                      </button> : null}>
                  {w.beds.length > 0 && (() => {
            ...
            return <div className="mb-3">
                  ...
                  <div className="flex flex-wrap gap-2">
                    ...
                    {w.beds.map(b => <span key={b._id} className={`inline-flex items-center gap-1.5 rounded-full px-3 py-1.5 text-xs font-semibold ${b.status === "vacant" ? "bg-emerald-50 text-emerald-700" : b.status === "occupied" ? "bg-red-50 text-red-700" : "bg-slate-100 text-slate-500"}`}>
                        ...
                        {(role === "nurse" || role === "admin") && b.status !== "occupied" && <button type="button" onClick={() => submitSetBedStatus(w._id, b._id, b.status === "maintenance" ? "vacant" : "maintenance")} className="ml-1 underline decoration-dotted underline-offset-2 hover:opacity-70" title={b.status === "maintenance" ? "Mark vacant" : "Mark under maintenance"}>
                            {b.status === "maintenance" ? "restore" : "maintain"}
                          </button>}
                        {role === "admin" && b.status === "vacant" && <button type="button" onClick={() => submitDeleteBed(w._id, b._id)} className="ml-1 text-red-600 hover:opacity-70" title="Remove bed">
                            ✕
                          </button>}
                      </span>)}
                  </div>
        ...
        {(role === "doctor" || role === "receptionist") && <div>
          ...
          {loading ? <SkeletonList count={3} /> : admissions.length === 0 ? <EmptyRow>No admissions match this filter.</EmptyRow> : <div className="space-y-4">
            ...
            value: new Date(a.dischargeDate).toLocaleString([], {
              ...
              timeStyle: "short"
                  ...
                  {a.status === "admitted" && (role === "nurse" || role === "doctor") && <div className="mt-4 flex flex-wrap items-center gap-2 pt-4 border-t border-mist">
              ...
              [a._id]: {
                ...
                bedId: e.target.value
                  ...
                  {a.status === "admitted" && role === "doctor" && <div className="mt-4 space-y-3 pt-4 border-t border-mist">
              ...
              [a._id]: {
                ...
                followUpInstructions: e.target.value
                  ...
                  {a.status === "discharged" && role === "receptionist" && <div className="mt-4 space-y-3 pt-4 border-t border-mist">
