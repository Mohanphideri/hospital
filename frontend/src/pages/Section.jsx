...
import { DataCard, DataGrid, StatusBadge, statusTone, EmptyRow, SectionToolbar, BillBreakdown } from "../components/DataCard";
...
import { SearchInput } from "../components/DataCard";
...
import { renderTicketThreadImpl } from "./sections/TicketThread.jsx";
import { renderMessagesBoardImpl } from "./sections/MessagesBoard.jsx";
import { renderAnnouncementsWorkspaceImpl } from "./sections/AnnouncementsWorkspace.jsx";
import { renderIpdWorkspaceImpl } from "./sections/IpdWorkspace.jsx";
import { renderSalarySlipsWorkspaceImpl } from "./sections/SalarySlipsWorkspace.jsx";
import { renderPatientContentImpl } from "./sections/PatientContent.jsx";
import { renderAmbulanceRequestsImpl } from "./sections/AmbulanceRequests.jsx";
...
export default function Section() {
  ...
  const [doctorApptDate, setDoctorApptDate] = useState(() => new Date().toISOString().slice(0, 10));
  ...
  // to). Only used/shown when the patient's profile doesn't already have one.
  ...
  const [leaveConflicts, setLeaveConflicts] = useState({}); // { [leaveId]: conflicts[] | null }
  const [leaveRejectDrafts, setLeaveRejectDrafts] = useState({}); // { [leaveId]: reasonText }
  const [leaveHistoryFilter, setLeaveHistoryFilter] = useState("all"); // "all" | "approved" | "rejected"
  const [allAppointments, setAllAppointments] = useState([]);
  const [analytics, setAnalytics] = useState(null);

  // --- Admin: audit log search (on-demand only - see runAuditSearch below.
  // Previously this fetched up to 500 logs unconditionally every time the
  // Overview loaded; now it stays empty until the admin picks a staff
  // member and/or date range and searches, which keeps the dashboard fast). ---
  ...
  // How many patients the next added time slot should allow (see
  // DoctorSlot.capacity) - defaults to 1 (one patient per slot, the
  // original behavior); admin can raise it for a group/walk-in-style block.
  ...
  const [dispenseDrafts, setDispenseDrafts] = useState({}); // { "rxId:index": { status, quantity, price } }
  ...
  const [bookPatientMode, setBookPatientMode] = useState("existing"); // "existing" | "new"
    ...
    const fetchData = async () => {
      try {
        if (section === "my-salary") {
          ...
          // Bespoke front-desk dashboard: a chosen day's appointments (today
          // by default, until the receptionist picks another date), live
          // walk-in queue (aggregated per-department from the existing queue
          // status endpoint), today's admissions, pending emergency requests
          // and unpaid bills - all from endpoints reception already has
          // access to elsewhere in the portal, just summarised here.
          const now = new Date();
          ...
          // Hospital-wide command center: reuse the same analytics overview
          // that powers the Analytics section, plus pending leave requests,
          // emergency requests and the latest announcements - everything an
          // admin needs at a glance, in the same KPI + tables layout used
          // for reception.
          ...
          // Doctor's own front page: today's patient list, own IPD patients,
          // tickets assigned to them and their leave status - same shape as
          // the reception dashboard, scoped to this doctor.
          const now = new Date();
          ...
          // Ward-facing front page: current admitted patients, bed
          // availability across wards, assigned tickets and leave status.
          ...
          // Every staff role already calls these two endpoints from their own
          // Tickets / Leave history sections - reusing them here to power a
          // couple of honest "home" KPIs instead of an empty quick-links hub.
          ...
          if (section === "appointments") {
            ...
            // Backend wraps the active token as { token: {...} } (or
            // { token: null } when the patient isn't in any queue right
            // now) - unwrap it here so PatientContent's queue view reads
            // token/department/estimatedWaitTime/status directly off
            // payload instead of off payload.token, which is what was
            // silently breaking the live queue / ETA display.
          ...
          if (section === "staff") {
            ...
            const [overviewRes, cashflowRes, ambulanceRes, staffRes] = await Promise.all([
              ...
              // Just for the audit-search staff dropdown - the logs
              // themselves are fetched on demand, see runAuditSearch.
  ...
  // Manual, on-demand refresh for the patient's Live queue card - explicitly
  // triggered by the "Refresh" button, kept as a fallback alongside the
  // automatic socket-driven updates below (e.g. for a dropped connection).
  ...
  const refreshQueueStatus = async () => {
    ...
    try {
      ...
      // a failed manual refresh just leaves the last-known state on screen
    } finally {
      setQueueRefreshing(false);
    }
  };

  // True live updates for the patient's queue card. The backend already
  // broadcasts a 'queue-status-updated' event to a department's socket room
  // whenever that queue actually changes (someone joins, is called, leaves,
  // etc - see controllers/queueController.js#broadcastQueueUpdate), but
  // nothing on the frontend was ever joining that room or listening for the
  // event, so the card never updated until the patient manually refreshed.
  // This joins the patient's own department room for as long as they have
  // an active token in it, and silently re-fetches on every push.
  useEffect(() => {
    ...
    const attach = () => {
      ...
      if (!socket) {
        // The socket is created at login/session-restore and may not exist
        // yet the instant this effect first runs - poll briefly rather than
        // silently missing live updates for the rest of the session.
      ...
      handler = (evt) => {
        ...
        queueService
          ...
          .catch(() => {
            // A missed live update isn't fatal - the Refresh button and the
            // next successful push both still work.
          });
      };
  ...
  // Same fix as above, for the receptionist front-desk dashboard's walk-in
  // queue table: join every department's room while that dashboard is open,
  // and on any push, refetch just that one department's queue (not the
  // whole dashboard - appointments/admissions/bills don't need to reload)
  // and splice it back into receptionHome.queueTokens.
  useEffect(() => {
    ...
    const attach = async () => {
      ...
      try {
        ...
        return; // no departments to subscribe to - nothing more to do
      }
      if (cancelled) return;
      ...
      handler = (evt) => {
        ...
        queueService
          ...
          .catch(() => {
            // A missed push just leaves that department's rows stale until
            // the next successful one, or the section is reopened.
          });
      };
      socket.on("queue-status-updated", handler);
    };
  ...
  // Viewing the emergency requests list marks any new arrivals as read, and any
  // request pushed in live over the socket while this list is open gets merged
  // straight into it (deduped by _id) rather than waiting for a manual refresh.
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    scheduleService
      ...
      .then((response) => {
        ...
        // Defensive normalization: always end up with { time, capacity }
        // objects here, even if a legacy slot document predates the
        // capacity field (capacity would come back undefined) - the editor
        // UI and saveDoctorSchedule's summary both assume this shape.
  ...
  const confirmAndBookSlot = async (time) => {
    ...
    const ok = await confirm({
      ...
      message: `Book ${time} on ${new Date(selectedDate).toLocaleDateString([], { dateStyle: "medium" })}? HeartStone will assign you the doctor available for that slot.`,
  ...
  // --- Staff (nurse / receptionist) actions ---
  ...
  // Case-insensitive exact match first (e.g. "Paracetamol" prescribed ==
  // "Paracetamol" in stock), falling back to a substring match for minor
  // naming differences (e.g. "ORS" prescribed vs "ORS Sachet" in stock).
  ...
  const runLookup = async (e) => {
    ...
    try {
      ...
      // Auto-fetch each medicine against current stock so the pharmacist
      // doesn't have to hunt for it manually - already-dispensed lines are
      // left alone since they're locked from further edits anyway.
      const linkUpdates = {};
      const draftUpdates = {};
      results.forEach((rx) => {
        (rx.medicines || []).forEach((med, i) => {
          ...
          if (!match) return; // stays "not available" - nothing in stock under this name
          const key = `${rx._id}:${i}`;
  ...
  // (see Department.isGeneral). Not tied to any doctor schedule - reception
  // can give any time and any doctor here.
  ...
  const submitBookAppointment = async () => {
    ...
    try {
      ...
      // Reset for the next booking, but keep department/date/time since that's likely reused.
  ...
  // General Consultation: no date/time/doctor needed up front - just the
  // patient and the (General) department. Creates a "pending" appointment;
  // assign a doctor + time afterwards from the Appointments section.
  ...
  // Nurse / admin: take a vacant bed in or out of maintenance.
  ...
  // ---------------------------------------------------------------------
  // Staff message board (every staff portal)
  // ---------------------------------------------------------------------
  ...
  // ---------------------------------------------------------------------
  // Public announcements (admin) - shown in the Notifications tab on Landing
  // ---------------------------------------------------------------------
  ...
  // Shared UI for the staff message board - called from every staff portal.
  ...
  // Admin-only UI for managing public homepage announcements.
  ...
  // ---------------------------------------------------------------------
  // Shared salary-slips workspace (admin) - generate a slip
  // for any staff member, including ward/ICU nurses and the receptionists
  // who field emergency/ambulance requests, and mark slips paid.
  // ---------------------------------------------------------------------
  ...
  const renderPatientContent = () => renderPatientContentImpl({ bookEmailInput, bookingStatus, cancelDrafts, cancelMyAppointment, cancelReasons, config, confirmAndBookSlot, current, departments, error, fetchSlots, loading, myEncounters, newTicketForm, newTicketStatus, openCancelId, patientProfile, patientProfileForm, patientProfileMessage, patientProfileSaving, patientReplyDrafts, patientReplyToTicket, payload, queueRefreshing, refreshQueueStatus, renderTicketThread, searchQuery, section, selectedDate, selectedDepartment, setBookEmailInput, setBookingStatus, setCancelDrafts, setError, setNewTicketForm, setOpenCancelId, setPatientProfileForm, setPatientReplyDrafts, setSearchQuery, setSelectedDate, setSelectedDepartment, setSlots, slots, submitNewTicket, submitPatientProfileUpdate, user });

  // Shared by both admin and receptionist - the request list, phone/location
  // details, and action buttons are identical for both roles; only the page
