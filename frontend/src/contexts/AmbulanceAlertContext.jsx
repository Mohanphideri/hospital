...
export function AmbulanceAlertProvider({ children }) {
  ...
  const [incoming, setIncoming] = useState([]); // newly-arrived requests, for lists to merge in
  ...
  useEffect(() => {
    ...
    // The socket may connect slightly after this effect runs (it's created on
    // login/session-restore) - poll briefly for it rather than missing the
    // very first event of the session.
    let cancelled = false;
    let attempts = 0;
    let handler;
      ...
      handler = (request) => {
        ...
        if (typeof Notification !== "undefined" && Notification.permission === "granted") {
          try {
            ...
            // Notification construction can fail on some platforms - non-critical.
          }
        }
      };
