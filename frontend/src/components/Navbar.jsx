...
export default function Navbar() {
  ...
  return (
    <>
      {/* Emergency utility bar - present on every page, like a real hospital's
          always-visible emergency line, not tucked inside a menu. */}
      ...
      <header className="sticky top-0 z-40 bg-paper/90 backdrop-blur border-b border-mist">
        ...
        {/* Mobile menu - without this, phone visitors only ever saw the logo
            and "Sign in": every nav link and quick action was invisible past
            the md breakpoint, which is most of this hospital's real traffic. */}
        {isLanding && mobileOpen && (
          <div className="md:hidden border-t border-mist bg-paper px-6 py-5 space-y-5">
            ...
            <div className="grid grid-cols-2 gap-3 pt-1 border-t border-mist">
              <Link
                to="/book-appointment"
                onClick={() => setMobileOpen(false)}
