...
export default function Sidebar({ config, user, sidebarVisible, onToggleSidebar, onSelectNavItem, mobileOpen, onCloseMobile, onLogout, loggingOut }) {
  ...
  // Static portal identity (role + desk name) - not the logged-in person's
  // name, which already appears in the top header's user menu. Falls back to
  // "<Label> Portal" / tagline for roles that don't define a dedicated
  // portalLabel, so only opted-in portals change appearance here.
  const portalLabel = config.portalLabel || `${config.label} Portal`;

  // The line under the portal label used to be a hardcoded placeholder per
  // role (e.g. always "Dr. Rina Kapoor · Cardiology" on the doctor portal),
  // so it could show a completely different name than whoever was actually
  // logged in. Build it from the real signed-in user instead, and only fall
  // back to the static config tagline if we somehow don't have a user yet.
                ...
                {!isCollapsed && (
                  <div className="space-y-1 mt-0.5">
                    {group.items.map((s) => {
                      ...
                      // Every role gets its own portal accent (patients: warm
                      // crimson; everyone else: the hospital's navy) instead
                      // of one generic indigo hardcoded for the whole app.
                      // Class names are written out in full (not built via
                      // template interpolation) so Tailwind's build-time
