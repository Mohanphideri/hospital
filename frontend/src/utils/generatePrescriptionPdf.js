 ...
 * @param {object} prescription - prescription document. If doctorId/patientId are
 *   populated objects, those are used directly (doctor & pharmacist/reception views).
 *   Otherwise the optional overrides below are used (patient's own portal view,
 *   where the prescription's patientId/doctorId may not be populated).
 * @param {object} [patientOverride] - { name, phone } fallback if prescription.patientId isn't populated
 * @param {object} [doctorOverride] - { name, designation, degree, registrationNo } fallback if prescription.doctorId isn't populated
 */
